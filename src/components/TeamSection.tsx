  ...
  return (
    <section id="equipo" className="section-padding">
      <div className="max-w-5xl mx-auto">
        <AnimatedSection>
          <p className="text-sm font-semibold tracking-widest uppercase text-primary mb-4">{t("team.label")}</p>
          <h2 className="text-3xl md:text-5xl font-bold mb-6">{t("team.title")}</h2>
          <p className="text-muted-foreground text-lg max-w-3xl leading-relaxed mb-12">{t("team.desc")}</p>
        </AnimatedSection>
        ...
        <AnimatedSection delay={0.1}>
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
            {teamMembers.map((m) => {
              const Icon = m.icon;
              return (
                <div key={m.id} className="group relative rounded-xl overflow-hidden border border-border h-56 sm:h-64">
                  <img src={m.bg} alt={m.role} className="absolute inset-0 w-full h-full object-cover group-hover:scale-110 transition-transform duration-700" />
                  ...
                  <div className="absolute bottom-0 left-0 right-0 p-4">
                    <h3 className="text-lg sm:text-xl font-bold text-foreground leading-tight">{m.name}</h3>
                    <p className="text-xs text-primary font-medium mt-1">{m.age} {t("team.years")}</p>
